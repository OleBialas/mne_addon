...
def global_field_power(data):
    """
    Compute the global field power which is the standard deviation of the squared evoked response.
    if input data is 3-dimensional it is considered as epoched data and averaged over the first dimension (number
    ...
    if data.ndim == 3:
        ...
        raise ValueError("Data must be either 2-dimensional (evoked responses) or 3-dimensional (epochs)")
    return np.std(evoked**2, axis=0)

def global_rms(data):
    """
    Compute the square-root of the mean over all channels after squaring (= root mean square).
    if input data is 3-dimensional it is considered as epoched data and averaged over the first dimension (number
    of epochs) to obtain the evoked response.
    """
    if data.ndim == 3:
