...
from mne.channels import read_montage, DigMontage
from mne import (set_eeg_reference, add_reference_channels,
...
def run_pipeline(raw, parameters):
    """
    Run all the steps of the preprocessing pipeline on the raw data.
    return processed epoched data, the result from the idependent component
    anaylsis and a log including the channels that were interpolated, the
    number of rejected epochs and the labels of the ica components marked as
    bad. The preprocessing steps take the following parameters (in a
    dict):
    """
    ...
    epochs, ica = \
        reject_ica(epochs, **parameters["ica_params"])
    log = dict(bad_channels=ransac.bad_chs_, n_reject=int(sum(log.bad_epochs)),
               bad_ica=ica.labels_)
    return epochs, log, ica


def filt_raw(raw, **filter_params):
    h = create_filter(data=raw._data[0], sfreq=raw.info["sfreq"],
                      **filter_params)
    for data, ch in zip(raw._data, range(raw.info["nchan"])):
        print("filter channel" + str(ch))
        data_filt = np.convolve(h, data)  # apply filter
        data_filt = data_filt[len(h) // 2:-len(h) // 2+1]  # compensate delay
        raw._data[ch] = data_filt
    return raw


def read_brainvision(fname, montage_path, apply_montage=True,
                     add_ref=True, preload=False):
...
def load_montage(path, plot=False):
    ...
    if not file:
        ...
        try:
            ...
            return None
        try:
            ch_names = json.load(open(Path(
                os.environ["EXPDIR"]+"cfg/eeg_channel_names.cfg")))
...
def reject_ica(inst, reference_ica, n_components=0.99, method="fastica",
               corr_thresh=0.9, plot=False):
    ...
    for component, label in zip(components, labels):
        corrmap([reference_ica, ica], template=(0, component[0]),
